...
| 文件/模块                | 主要功能简介                                               |
|-------------------------|----------------------------------------------------------|
| `../../run_crypto_crawler.py` | 主爬虫入口，采集 Telegram 等渠道的加密新闻（位于项目根目录） |
...
| `../../config.py`       | 配置文件，定义爬虫参数、API 密钥、数据库路径等              |
...
1. 安装依赖包（建议使用 requirements.txt）
   ...
   ```
2. 配置 Telegram API 密钥、数据库路径等（编辑项目根目录的 `config.py`）
...
- **配置文件**：项目根目录的 `config.py` 支持多渠道参数，建议每次部署前检查

---

## 6. 推荐工作流

1. 配置好 `config.py` 和依赖环境
...
- **数据库连接异常**：确认 `config.py` 中的数据库路径配置正确
